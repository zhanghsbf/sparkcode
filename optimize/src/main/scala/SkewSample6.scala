...
object SkewSample6 {
   ...
   */
  def skew6():Unit = {
    val conf = new SparkConf().setAppName("DataSkewTest06").setMaster("local[4]")
...
//    val rawRDD = spark.textFile("/root/data/skewdata.csv")//读取数据源
     ...
     */
    reducedRDD_03.cache()
    val keys: Array[String] = reducedRDD_03.map(kv => kv._1).distinct().collect()

    val finalRDD = reducedRDD_03.reduceByKey(new MyMapPartitioner(keys), (map1,map2) => { /**合并2个map中的元素，key相同则value值相加*/
      //将map1和map2中的结果merge到map3中，相同的key，则value相加
      val map3 = new util.HashMap[String,Int](map1)
      val value: util.Iterator[Entry[String, Int]] = map2.entrySet().iterator()
      while(value.hasNext){
